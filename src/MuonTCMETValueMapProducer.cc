...
namespace cms {
  MuonTCMETValueMapProducer::MuonTCMETValueMapProducer(const edm::ParameterSet& iConfig) {
    ...
    muonQoverPerror_ = iConfig.getParameter<double>("qoverpError_muon"); 
    muondeltaPt_     = iConfig.getParameter<double>("deltaPt_muon"    );
  ...
  void MuonTCMETValueMapProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    MuonTCMETValueMapProducer::response_function = getResponseFunction();
  ...
  bool MuonTCMETValueMapProducer::isGoodMuon( const reco::Muon* muon ) {
    ...
    if( globalTrack->qoverpError() > muonQoverPerror_ ) return false;

    else return true;
  }

  // ------------ check if global and tracker muon in good agreement  ------------
  bool MuonTCMETValueMapProducer::isGoodGlobalMuon( const reco::Muon* muon ) {

    double deltaPt = fabs( muon->innerTrack().get()->pt() - muon->pt() );
  
    if( ( deltaPt / muon->innerTrack().get()->pt() ) > muondeltaPt_ ) return false;
  ...
  bool MuonTCMETValueMapProducer::isGoodTrack( const reco::Muon* muon ) {
    ...
    if( fabs( outerTrkPosition.Phi() ) > 2 * TMath::Pi() ) return false;
    if( fabs( outerTrkPosition.Eta() ) > 5 ) return false; 
  ...
  // ------------ single pion response function  ------------
  TH2D* MuonTCMETValueMapProducer::getResponseFunction( ) {
